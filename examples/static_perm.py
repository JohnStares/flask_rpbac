...
from setup import Perm, Post, User, create_db_data, db, jwt, rpbac

from flask_rpbac import Permission
...
@app.get("/posts")
@jwt_required()
@rpbac.permission_required(Permission(Perm.POST_READ))
def get_posts():
...
# Only users with edit permission can make changes to this resource
@app.patch("/post/<id>")
@jwt_required()
@rpbac.permission_required(Permission(Perm.POST_EDIT))
def edit_post(id):
    data = request.get_json()
...
# Only users with delete permissions can delete a post
@app.delete("/post/<id>")
@jwt_required()
@rpbac.permission_required(Permission(Perm.POST_DELETE))
def delete_post(id):
    post = db.session.get(Post, id)
...
# Note: When assigning multiple permissions to the Permission class set match="any" by default it is set to "all". it all depends on your use case
# match="all" means a user must have all the permissions while match="any" means a user can have any of the permissions.
# e.g Permission(Perm.POST_EDIT, POST.POST_READ, match="any")
