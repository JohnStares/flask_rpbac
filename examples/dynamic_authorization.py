...
from flask_rpbac import Any, Predicate, Role, RPBACBuildContext
...
# Some functions that permits what a user can do
def own(ctx: RPBACBuildContext):
    post_id = ctx.kwargs["id"]

    post = db.session.get(Post, post_id)

    return bool(f"{post.user_id}" == get_jwt_identity() if post is not None else False)
    # You can decide on how you want to handle empty posts
...
# users can edit their own post or any user with admin can edit
@app.patch("/post/<id>")
@jwt_required()
@rpbac.required(Any(Role(Rolee.ADMIN), Predicate(own)))
def edit_post(id):
    data = request.get_json()
...
# Users can delete their own post or any one with admin role can
@app.delete("/post/<id>")
@jwt_required()
@rpbac.required(Any(Role(Rolee.ADMIN), Predicate(own)))
def delete_post(id):
